...
class Configuration(BaseModel):
    ...
    Densité_apparente_des_motes: Optional[float] = None
    Densité_apparente_du_sol: Optional[float] = None
    ...
    @validator('Densité_apparente_des_motes', 'Densité_apparente_du_sol', always=True)
    def check_motes_and_sol(cls, v, values, field):
        if 'Densité_apparente_des_motes_Bool' in values and values['Densité_apparente_des_motes_Bool'] is True:
            if 'Densité_apparente_des_motes' not in values or 'Densité_apparente_du_sol' not in values or values['Densité_apparente_des_motes'] is None or values['Densité_apparente_du_sol'] is None:
                raise ValueError(
                    f'Both {field.alias} and the other field must be provided when Densité_apparente_des_motes_Bool is True')
        return v
...
def bilan_hydrique():
    ...
    daily_data['KC'] = daily_data.index.map(lambda x: calculate_KC_values(
        configuration.Plant_name, x.strftime('%Y-%m-%d')))

    # Add ETR values column
    daily_data['ETR'] = daily_data['ETP'] * daily_data['KC']

    # Add lg_mod Values column
    perte_d_ensoleillement = configuration.Taux_de_couverture * \
        0.8 - (configuration.Hauteur_de_linstallation - 3.5) * 2.95
    ...
    if configuration.Densité_apparente_des_motes != None and configuration.Densité_apparente_du_sol != None:
            ...
            if keys[0] <= configuration.Densité_apparente_des_motes <= keys[1]:
                ...
                for key_float in teneurSol[configuration.Texture_du_sol][keys]:
                    absolute_difference = abs(
                        key_float - configuration.Densité_apparente_des_motes)
                ...
                if absolute_values[0] < absolute_values[1]:
                    ...
                    Reserve_Utile_maximun = (
                        Teneur_eau_sol * 10000 / 1000) * configuration.Profondeur_des_racines * (1 - (configuration.Pierrosité / 100))
        ...
        for keys in teneurSol[configuration.Texture_du_sol]:
            for key_float in teneurSol[configuration.Texture_du_sol][keys]:
                RU_list.append(
                    teneurSol[configuration.Texture_du_sol][keys][key_float]["RU"])
        RU_mediane = sum(RU_list) / len(RU_list)
        Reserve_Utile_maximun = (RU_mediane * 10000 / 1000) * \
            configuration.Profondeur_des_racines * \
            (1 - (configuration.Pierrosité / 100))
        Reserve_dificilement_utilisable = 1 / 3 * Reserve_Utile_maximun
    ...
    daily_data["capacité_au_champs"] = Reserve_Utile_maximun
    ...
    daily_data.loc[configuration.start_date,
                   "eau_utile"] = daily_data.loc[configuration.start_date, "capacité_au_champs"]
    daily_data.loc[configuration.start_date,
                   "eau_utile_pv"] = daily_data.loc[configuration.start_date, "capacité_au_champs"]

    for index, row in daily_data.iterrows():
        if daily_data.loc[index, "eau_utile"] == daily_data.loc[configuration.start_date, "capacité_au_champs"]:
            continue
        else:
            previous_date = (index - pd.DateOffset(days=1)
                             ).strftime('%Y-%m-%d')
            ...
            daily_data.loc[index, "eau_utile"] = calculate_eau_utile_value(daily_data.loc[previous_date, "eau_utile"], daily_data.loc[previous_date, "precipitation"], daily_data.loc[
                                                                           previous_date, "irrigation"], daily_data.loc[previous_date, "ETR"], daily_data.loc[previous_date, "capacité_au_champs"], daily_data.loc[previous_date, "RDU"])
            ...
            daily_data.loc[index, "eau_utile_pv"] = calculate_eau_utile_value(daily_data.loc[previous_date, "eau_utile_pv"], daily_data.loc[previous_date, "precipitation"], daily_data.loc[
                                                                              previous_date, "irrigation_pv"], daily_data.loc[previous_date, "ETR_PV"], daily_data.loc[previous_date, "capacité_au_champs"], daily_data.loc[previous_date, "RDU"])
    daily_data.loc[configuration.end_date, "irrigation"] = 2/3 * daily_data.loc[configuration.end_date, "RDU"] if (
        daily_data.loc[configuration.end_date, "eau_utile"] + daily_data.loc[configuration.end_date, "precipitation"] - daily_data.loc[configuration.end_date, "ETR"]) < 2/3 * daily_data.loc[configuration.end_date, "RDU"] else 0
